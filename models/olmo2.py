...
def forward_layer(cfg, x, w, kv=None, pos=0):
    ...
    # QKV projection
    q = jnp.einsum('btd,nhd->btnh', x, w['q_proj'], preferred_element_type=x.dtype, out_sharding=P('data', None, 'model', None))
    k = jnp.einsum('btd,khd->btkh', x, w['k_proj'], preferred_element_type=x.dtype, out_sharding=P('data', None, 'model', None))
    v = jnp.einsum('btd,khd->btkh', x, w['v_proj'], preferred_element_type=x.dtype, out_sharding=P('data', None, 'model', None))

    # Q/K norm
    q = rms_norm(q, w['q_norm'], cfg['rms_norm_eps'], axis=(-2, -1))
    k = rms_norm(k, w['k_norm'], cfg['rms_norm_eps'], axis=(-2, -1))
    ...
    x += rms_norm(o, w['post_attention_layernorm'], cfg['rms_norm_eps'])
    
    # MLP
    gate = jax.nn.silu(jnp.einsum('btd,fd->btf', x, w['gate_proj'], preferred_element_type=jnp.float32, out_sharding=P('data', None, 'model')))
    up = jnp.einsum('btd,fd->btf', x, w['up_proj'], preferred_element_type=x.dtype, out_sharding=P('data', None, 'model'))
    mlp_out = jnp.einsum('btf,df->btd', gate * up, w['down_proj'], preferred_element_type=x.dtype, out_sharding=P('data', None, None))

    # post-mlp norm + residual
    x += rms_norm(mlp_out, w['post_feedforward_layernorm'], cfg['rms_norm_eps'])
...
def load(model_id='allenai/OLMo-2-0425-1B', tp_devices=1, hf_ckpt_dir='~/weights/huggingface'):
    ...
    L, N, K, D = cfg['num_hidden_layers'], cfg['num_attention_heads'], cfg['num_key_value_heads'], cfg['hidden_size']
    H = D // N
